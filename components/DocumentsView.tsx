...
import { FileText, Download, Printer, Copy, CheckCircle, AlertTriangle, Calendar, User, FileOutput, Bus, Upload, ClipboardList, Award, Activity } from 'lucide-react';
...
type DocumentType = 'INCIDENCIA' | 'CITATORIO' | 'FICHA_DESCRIPTIVA' | 'PLANEACION' | 'ACTA_HECHOS' | 'PERMISO_SALIDA' | 'AUTORIZACION_EVENTO' | 'PRESENTACION_RESULTADOS' | 'OBSERVACIONES_BOLETA' | 'INFORME_PADRES' | 'INFORME_ACTIVIDADES' | 'LISTA_ASISTENCIA_PADRES' | 'PLAN_REZAGO' | 'REPORTE_RENDIMIENTO_GRUPO' | 'LISTA_ENTREGA_BOLETAS' | 'SEMAFORO_ESCOLAR';
            ...
            if (includeGroupSheet) {
                ...
                try {
                    ...
                    const groupData = {
                        ...
                        teacherName: students[0] ? getTeacherForStudent(config, students[0].group) : config.teacherName,
                        groupName: students[0] ? (students[0].group || config.gradeGroup) : config.gradeGroup,
            ...
            for (let i = 0; i < selectedStudents.length; i++) {
                ...
                try {
                    ...
                    const studentData = {
                        ...
                        keywords: keywords,
                        teacherName: getTeacherForStudent(config, student.group),
                        gradeGroup: student.group || config.gradeGroup,
                        groupName: student.group || config.gradeGroup,
                        schoolName: config.schoolName,
...
`;
        ...
        const data = {
            ...
            guardianName,
            schoolName: config.schoolName,
            teacherName: student ? getTeacherForStudent(config, student.group) : (students[0] ? getTeacherForStudent(config, students[0].group) : config.teacherName),
            gradeGroup: student ? (student.group || config.gradeGroup) : (students[0] ? (students[0].group || config.gradeGroup) : config.gradeGroup),
            groupName: student ? (student.group || config.gradeGroup) : (students[0] ? (students[0].group || config.gradeGroup) : config.gradeGroup),
                    ...
                    <div className="glass-card p-6 rounded-2xl">
                        ...
                        <div className="space-y-2">
                            {[
                                ...
                                { id: 'SEMAFORO_ESCOLAR', label: 'Semáforo de mi Escuela', icon: Activity },
                    ...
                    <div className="glass-card p-6 rounded-2xl">
                        ...
                        <div className="space-y-4">
                            {(selectedType === 'FICHA_DESCRIPTIVA' || selectedType === 'OBSERVACIONES_BOLETA') && (
                             ...
                             {selectedType !== 'PLANEACION' && selectedType !== 'PRESENTACION_RESULTADOS' && selectedType !== 'LISTA_ASISTENCIA_PADRES' && selectedType !== 'LISTA_ENTREGA_BOLETAS' && selectedType !== 'PLAN_REZAGO' && selectedType !== 'REPORTE_RENDIMIENTO_GRUPO' && selectedType !== 'SEMAFORO_ESCOLAR' && (
                                <>
